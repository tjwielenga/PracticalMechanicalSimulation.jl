...
model {
    name = "large_van_bristle_experiment",
    title = "Large Van - Experimental Bristle Tires",
    dimension = "spatial"
}
...
-- Reuse the released van's static geometry. The expression-tire result does
-- not contain bristle states, so the new shear states begin unstrained.
initial_conditions {
...
local van = large_van {
    ...
    tire_tangential_model = "bristle",
    -- Provisional curves centered on the saved van's 3.7--6.0 kN tire loads.
    -- They preserve approximately the old nominal stiffness but add the
    -- sublinear load sensitivity expected from a pneumatic tire.
    tire_patch_length_by_load = {
        {0, 0}, {2000, 0.15}, {4000, 0.21},
        {6000, 0.26}, {8000, 0.30}, {10000, 0.33}
    },
    tire_cornering_stiffness_by_load = {
        {0, 0}, {2000, 8500}, {4000, 16000},
        {6000, 23000}, {8000, 29500}, {10000, 35500}
    },
    tire_longitudinal_slip_stiffness_by_load = {
        {0, 0}, {2000, 2300}, {4000, 3500},
        {6000, 4300}, {8000, 4900}, {10000, 5400}
    },
    tire_shear_release_time = 0.01,
    tire_lateral_relaxation_fraction = 0.5,
    tire_friction = 1.0,
    forward_speed = 30.0,
    roof_contacts = true,
    colors = {
        body = "gray70", control_arm = "steelblue",
        spindle = "darkorange", stabilizer = "firebrick",
        stabilizer_link = "orange", steering_link = "gray35",
        tie_rod = "seagreen", steering_wheel = "black",
        axle = "gray35", leaf = "steelblue", shackle = "darkorange",
        tire = "gray20"
    }
}
