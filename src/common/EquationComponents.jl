"""Dimension-independent user scalar algebraic and differential equations."""
module EquationComponents

using ..AutomaticAnalysis
using ..InputUnits: angle_value
using ..ScalarExpressions

export UserEquationComponent, equation_component_registration,
    allocated_equation_component, initialize_equation_component!,
    equation_state_rates!, set_equation_stage!, equation_executable_blocks

struct UserEquationComponent
    name::Symbol
...
function equation_component_registration(name::Symbol, table)
...
function compile_component_equality(source, parameters, layout, aliases)
    ...
    left = compile_component_law(strip(parts[1]), parameters, layout, aliases)
    right = compile_component_law(strip(parts[2]), parameters, layout, aliases)
    (left, right)
end
...
function allocated_equation_component(layout, name, table, global_parameters)
    ...
    UserEquationComponent(name, state_indices, state_modes, state_rows,
        state_laws, algebraic_indices, algebraic_rows, algebraic_laws,
        Ref(:dynamic))
end

function initialize_equation_component!(initial, component, table)
...
function set_equation_stage!(component, stage)
    component.stage[] = stage
    component
end

function equation_state_rates!(derivative, component, t, z)
...
function equation_executable_blocks(component::UserEquationComponent)
